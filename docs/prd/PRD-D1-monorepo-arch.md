...
| 2026-08-18 | D4b 实施：DecorationStore（Monaco deltaDecorations 语义：reveal/搜索/断点/诊断）+ Gutter（行号槽 on/off/relative + glyph margin 断点红点/诊断图标 + 当前行高亮）+ View 集成（setDecorationProvider / setGutterEnabled / setGutterMarker / setCurrentLine） | FR4.4/4.5 落地，TC-D4-05/07 + FUNC-02/03/04/11 通过（view 29 测试全绿、284 全量） |
| 2026-08-18 | D5 实施：编辑能力——ViewCursors（合成层光标 + 选区 overlay + 跨行 block）+ InputHandler（键盘/IME 组合/剪贴板 → applyEdits + EditStack 合并 + 多光标；方向键/Home/End/PageUp/PageDown/Tab 缩进/undo-redo）+ ViewController 命令路由 | FR5.1~5.3 落地，TC-D5-01/02/03/04/06/07 通过（view 43 测试全绿、298 全量）；编辑模式 diff（FR5.4）在 D7 react 集成时接线 |
| 2026-08-18 | D6 实施：Minimap（canvas 缩略渲染 + token 色采样 + 视口框 + 滚动同步 + 点击/拖拽定位 + 断点/诊断标记）+ OverviewRuler（断点/诊断/搜索/光标投影 + 点击定位 + 可开关） | FR6.1~6.4 落地，TC-D6-01/02/03/04/05/06 通过（view 51 测试全绿、306 全量） |
| 2026-08-18 | D7 实施：@cd/react 集成——CodeDiff（props 与 1.3.0 兼容：三视图/reveal 高亮/工具栏骨架）+ CodeEditor（Monaco 式 API：model|value 双模式、undo/redo/focus/getValue/setValue、onChange/光标事件、minimap/gutter/诊断注入）+ model-registry（createModel/getModel/setModelLanguage/disposeModel）+ diagnostics（setModelMarkers owner 隔离） | FR7.1~7.6 落地，TC-D7-01/02/03/04/05/06 + FUNC-15/16 + COMP-01 通过（react 4 测试全绿、310 全量）；编辑模式 diff（FR5.4）接线完成（onChange → 外部重算） |
| 2026-08-18 | D8 实施：demo 迁移新架构（src/demo-new.tsx + index-new.html）+ View 容器撑满修复 + 性能基准达标（PERF-03 16ms / PERF-04 卡顿 0 / PERF-01 DOM 33↔41 / PERF-02 切换 40ms 含 diff 重算） | FR8.1~8.3 落地（demo 迁移 + 基准）；旧 src/code-diff 保留对照，移除与发布 2.0.0 待用户确认后执行 |
