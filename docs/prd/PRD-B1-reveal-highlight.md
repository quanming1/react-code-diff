...
|---|---|---|
| 2026-08-17 | 初始定稿 | — |
| 2026-08-17 | 开发中发现并修复 `virtual-v2.scrollToIndex` 的赋值钳制问题：展开折叠段后 totalHeight 提交滞后于滚动赋值，浏览器将 scrollTop 钳制为 0 导致跳转丢失——改为赋值后校验落位、被钳制则逐帧重试（至多 10 帧）。技术方案 §3 补记 | e2e 验证发现（demo 实测跳转不生效）；属 B1 交付范围内的引擎缺陷 |
